...
namespace GraphEngine {
    namespace Display {
        ...
        void CConvOffset::Gen(GUnits offset, const GPoint** points, int* pointCounts, int count)
        {
            ...
            for(int part = 0, off = 0; part < count; part++)
            {
                ...
                if(pointCounts[part] == 2)
                {
                    if(CalcSegment(inPoints[off], inPoints[off + 1], offset, beg, end))
                    {
                ...
                for(int point = off, sz = off + pointCounts[part] - 2; point < sz; point++)
                {
                    ...
                    if(!CalcSegment(inPoints[point], inPoints[second_point], inPoints[third_point], offset, beg, end, true, &use_both))
                        continue;
                ...
                if(ok && CalcSegment(inPoints[first_point], inPoints[second_point], inPoints[third_point], offset, beg, end, false, &use_both))
                {
                    if(outPointOffset + 2 > pointBufferSize)
                    {
                        pointBufferSize = outPointOffset + 2;
        ...
        bool CConvOffset::CalcSegment(const GPoint& p1,
                                       const GPoint& p2,
                                       const GPoint& p3,
                                       GUnits offset,
                                       GPoint& beg,
                                       ...
                                       bool *use_both)
        {
            *use_both = false;
            ...
            if(!GenLineKoef(p1, p2, &A1, &B1, &C1))
                return false;
            if(!GenLineKoef(p2, p3, &A2, &B2, &C2))
                return false;

            if(IsParallel(A1, B1, A2, B2) && !IsSameOrientation(A1, B1, A2, B2))
                *use_both = true;
            ...
            if(*use_both || IsParallel(A1, B1, A2, B2))
            {
                if(first_seg)
                {
        ...
        bool CConvOffset::GenLineKoef(const GPoint& p1, const GPoint& p2, double* a, double* b, double* c)
        {
            if(p1 == p2)
                return false;
        ...
        bool CConvOffset::IsParallel(double A1, double B1, double A2, double B2)
        {
            const double e = std::numeric_limits<double>::epsilon() * 10;
            return fabs(A1*B2 - A2*B1) < e;
        }

        bool CConvOffset::IsSameOrientation(double A1, double B1, double A2, double B2)
        {
            const double e = std::numeric_limits<double>::epsilon() * 10;
            return (fabs(A1 - A2) < e) && (fabs(B1 - B2) < e);
        }

        bool CConvOffset::Cross(double A1, double B1, double C1, double A2, double B2, double C2, GPoint& p)
        {
        ...
        double CConvOffset::Len(const GPoint& p1, const GPoint& p2)
        {
