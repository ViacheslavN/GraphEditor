...
namespace GraphEngine {
    namespace Display {
        ...
        void CVertexSrc::Attach(const GPoint* pPoints, const int* pPointCounts, int count)
        {
            m_pDasher = 0;
            m_pPoints = pPoints;
            m_pEllipse = 0;
        ...
        void CVertexSrc::Attach(agg::ellipse& ellipse)
        {
            m_pEllipse = &ellipse;
            m_pRounded_rect = 0;
        }
        void CVertexSrc::Attach(agg::rounded_rect& rounded_rect)
        {
            m_pRounded_rect = &rounded_rect;
            m_pEllipse = 0;
        }
        void CVertexSrc::rewind(unsigned path_id)
        {
            if(m_pEllipse)
            {
        ...
        unsigned CVertexSrc::vertex(double *x, double *y)
        {
            if(m_pEllipse)
            {
                double x_, y_;
            ...
            *x = m_pPoints[m_nIdx].x + m_nOffsetX + 0.5;
            *y = m_pPoints[m_nIdx].y + m_nOffsetY + 0.5;
        ...
        void CVertexSrc::SetOffset(GUnits offsetX, GUnits offsetY)
        {
            m_nOffsetX = offsetX;
            m_nOffsetY = offsetY;
        }

        void CVertexSrc::GetOffset(GUnits* offsetX, GUnits* offsetY)
        {
            *offsetX = m_nOffsetY;
            *offsetY = m_nOffsetY;
        }
